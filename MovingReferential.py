...
class MovingReferential1(Scene):
    def construct(self):
        #plane = NumberPlane()
        #self.add(plane)
        bloc1 = Square(side_length=1)
        bloc2 = Square(side_length=1)
        pulley = Circle(radius=0.5, fill_opacity=1)
        inclinedPlane = Polygon([-5, -2, 0], [5, -2, 0], [0, 0, 0])
        ...
        xaxis = Arrow(start=ORIGIN, end=RIGHT, buff=0)
        x_label = Text("x")
        x_label.scale(0.65)
        x_label.move_to([0.85, -0.35, 0])

        yaxis = Arrow(start=ORIGIN, end=[0, 1, 0], buff=0)
        y_label = Text("y")
        y_label.scale(0.65)
...
class MovingReferential2(Scene):
    def construct(self):
        plane = NumberPlane()
        self.add(plane)
        bloc1 = Square(side_length=0.75)
        bloc2 = Square(side_length=0.75)
        pulley = Circle(radius=0.5, fill_opacity=1)
        ...
        rope1 = Line(p1, [-0.5, 2, 0])
        d1 = Dot(point=p1).set_color(BLUE)
        p2 = bloc2.get_corner(UP)
        rope2 = Line([0.5, 2, 0], p2)
        d2 = Dot(point=p2).set_color(RED)
...
class MovingReferential3(Scene):
    def construct(self):
        ...
        rope1 = Line(p1, [2, 0.5, 0])
        d1 = Dot(point=p1).set_color(BLUE)
        p2 = bloc2.get_corner(UP)
        rope2 = Line([2.5, 0, 0], p2)
        d2 = Dot(point=p2).set_color(RED)
